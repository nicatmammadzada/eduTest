    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <div class="heading-elements">
                <a href="{{route('admin.blog.create')}}"><span class="label label-success">YENİ POST</span></a>
            </div>
        </div>
            ...
            @if($posts->count()>0)
                @foreach($posts as $key=>$post)
                    <tr>
                        ...
                        <td class="bbb"><a href="{{route('admin.blog.edit',$post->slug)}}">{{$post->title}}</a></td>
                            ...
                            <ul class="icons-list">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a href="{{route('admin.blog.edit',$post->slug)}}"><i
                                                    class="icon-database-edit2"></i> Yenilə</a></li>
                                        <li>
                                            <a onclick='checkDeleteConfrim("{{route('admin.blog.destroy',$post->slug)}}")'><i
...
@section('foot')
    ...
    <script>
        function choose1(id, event) {
            var url = '{{route('admin.blog.choose1')}}';
    ...
    <script>
        function choose2(id, event) {
            var url = '{{route('admin.blog.choose2')}}';
            checked(id, event, url)
        }
    </script>
    ...
    <script>
        function eventt(id, event) {
            var url = '{{route('admin.blog.event')}}';
            checked(id, event, url)
        }
    </script>
    ...
    <script>
        function analitika1(id, event) {
            var url = '{{route('admin.blog.analitika1')}}';
            checked(id, event, url)
        }
    </script>
    <script>
        function analitika2(id, event) {
            var url = '{{route('admin.blog.analitika2')}}';
            checked(id, event, url)
        }
    </script>
    <script>
        function mobile(id, event) {
            var url = '{{route('admin.blog.mobile')}}';
            checked(id, event, url)
        }
    </script>
    <script>
